...
export default class DataBoundFlexScrollView extends FlexScrollView {
    ...
    constructor(OPTIONS = {}){
        ...
        if (!this.options.sortingDirection) this.options.sortingDirection = 'ascending';
    ...
    _moveItem(oldId, prevChildId=null) {
        ...
        let previousSiblingIndex = this._getNextVisibleIndex(prevChildId); //prevChildId ? this._getDataSourceIndex(prevChildId) : this._getBeginPosition();

        /*
        while (previousSiblingIndex==-1) {
            previousSiblingIndex = this._getNextVisibleIndex(prevChildId);
        }*/
